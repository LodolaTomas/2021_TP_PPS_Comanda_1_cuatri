...
export class AdmBebidasPage implements OnInit {
  ...
  constructor(
    ...
    firestore.GetAll('usuarios').subscribe((data) => {
      this.usuarios = data;
      this.traerUsuario();
    });

    firestore.GetAll('pedidos').subscribe((data) => {
      this.pedidos = data;
    ...
    this.pedidos.forEach((uno) => {
      if (uno.status == 'preparando') {
        ...
        this.notifSVC.notifyByProfile(
          ...
          this.usuarioLog,
          'cocinero'
        ); //Mensaje, usuario logeado, y perfiles a notificar
      }
    });
  }
  ...
  Aceptar(pedido) {
    this.firestore.Update(pedido.value[0].idTable, 'pedidos', {
      statusBartender: true,
    });
  }
